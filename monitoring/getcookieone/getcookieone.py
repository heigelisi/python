	...
	def login(self,url,username,password,loginnum=1):
		...
		try:
			...
			while True:
				...
				try:
					message = browser.find_element(By.CSS_SELECTOR,'a[href*="action=logout"]').get_attribute('innerHTML')
					tuichu = 1
				except Exception as e:
					pass
			...
			#判断是否登录成功
			if username.lower() in message.lower() or '修改密码' in message or '修改密碼' in message or tuichu:
				#登录成功
