			...
			with open(self.path+'config.ini','r',encoding='utf8') as f:
				for line in f.readlines():
					...
					if '=' in line and class_ == 'database':
						conf = line.split('=')
						self.database[conf[0].strip()] = conf[1].strip()
					#公共
	...
	def updatedata(self,browser,url):
		...
		try:
			...
			try:
				...
				button = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,"button[name='profilesubmitbtn']")))
			except Exception as e:
				pass
			...
			#个人签名 sightml
			try:
				sightml = browser.find_element(By.CSS_SELECTOR,"textarea[name='sightml']")
				sightml.send_keys(self.registered_['sightml'])
			except Exception as e:
				pass

			#自我介绍 bio
			try:
				bio = browser.find_element(By.CSS_SELECTOR,"textarea[name='bio']")
				bio.send_keys(self.registered_['bio'])
			except Exception as e:
				pass
				
			#兴趣爱好 interest
			try:
				interest = browser.find_element(By.CSS_SELECTOR,"textarea[name='interest']")
				interest.send_keys(self.registered_['interest'])
			except Exception as e:
				pass
