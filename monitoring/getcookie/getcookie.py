...
class MonitoringDZ(object):
		...
		try:
			...
			with open(self.path+'config.ini','r',encoding='utf8') as f:
				for line in f.readlines():
					...
					if '=' in line and class_ == 'getcookie':
						conf = line.split('=')
						self.getcookie[conf[0].strip()] = conf[1].strip()
		...
		try:
			...
			while True:
				...
				try:
					...
					# self.showlog(browser,url+"\t失败，无法登陆此网站\r")
					continue
			# <a href="member.php?mod=logging&amp;action=logout&amp;formhash=225657b2">退出</a>
			#等待点击确定登陆
			i = 1800
			while True:
				message = ''
				...
				try:
					message = browser.find_element(By.CSS_SELECTOR,'a[href*="action=logout"]').get_attribute('href')
				except Exception as e:
					pass
			...
			#判断是否登录成功
			if username.lower() in message.lower() or 'action=logout' in message:
				#登录成功
			...
			# print(e)
			self.showlog(browser,url+"\t获取cookie失败！")
			# print(e)
		time.sleep(1)	
	...
	def main(self):
		...
		if not cursor:
			...
			global myqueue
			myqueue = queue.Queue(maxsize = int(self.getcookie['thrnumber']))
			joinlist = []
			infoid = 0
			while True:
				try:
