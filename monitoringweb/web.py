...
def add():
	if request.method == 'POST':
		...
		conn = sqlite3.connect(db)
		c = conn.cursor()
		sql = """insert into info(type,name,url,registered,listen,message,friends,hello,username,password,email,note,addtime) values(?,?,?,?,?,?,?,?,?,?,?,?,?)"""
...
def update():
	...
	if request.method == 'POST':
		...
		sql = """update info set type = ?,name = ?,url = ?,registered = ?,listen = ?,message = ?,friends = ?,hello = ?,username = ?,password = ?,email = ?,note = ?,perform = ? where id = ?"""
		res = c.execute(sql,(datas))
		if res:
			conn.commit()
			conn.close()
		...
		sql = "select id,type,name,url,registered,listen,message,friends,hello,username,password,email,note,perform from info where id = "+id_
		c.execute(sql)
		data = c.fetchall()
		print(data[0])
		datas = []
...
def index():
	...
	conn = sqlite3.connect(db)
	c = conn.cursor()
	sql = "select * from info "+where+' limit %s,30'%limit
	c.execute(sql)
	data = c.fetchall()
	sql2 = "select count(1) from info "+where
	c.execute(sql2)
	count = c.fetchall()
	...
	return flask.render_template('index.html',data=data,screening=screening,checked=checked,q=q,page=page,count=count[0][0],p=int(p),url=url,isreg='无法注册',title="可注册列表")
...
def status():
		if request.method == 'POST':
			...
			sql = "select %s from info where id = %s"%(parameter_[0],parameter_[1])
			conn = sqlite3.connect(db)
			c = conn.cursor()
			c.execute(sql)
			status_ = c.fetchall()[0][0]
			sql2 = "update info set %s = %s where id = %s"%(parameter_[0],str(int(not int(status_))),parameter_[1])
			res = 0
			if c.execute(sql2):
				conn.commit()
				res = 1
...
def getcookieone():
	if request.method == 'POST':
		...
		sql = "select url,username,password from info where id = "+str(id_)
		conn = sqlite3.connect(db)
		c = conn.cursor()
		c.execute(sql)
		data = c.fetchall()[0]
...
def updatecookie():
	...
	updatesql1 = "update info set cookie = 1 where id in (%s)"%(cookie1.strip(','))
	updatesql0 = "update info set cookie = 0 where id in (%s)"%(cookie0.strip(','))
...
def updateinput():
	...
	conn = sqlite3.connect(db)
	c = conn.cursor()
	sql = "update info set %s = '%s' where id = %d"%(type_,input_,int(id_))
	c.execute(sql)
	conn.commit()
	return '1'
...
def notreg():
	if request.method == 'POST':
		id_ = request.form.get('id')
		sql = "insert into info2(perform,seal,type,name,url,registered,listen,message,friends,hello,username,password,email,note,addtime,cookie) select perform,seal,type,name,url,registered,listen,message,friends,hello,username,password,email,note,addtime,cookie from info where id = "+str(id_)
		conn = sqlite3.connect(db)
		c = conn.cursor()
		if c.execute(sql):
			conn.commit()
			delsql = "delete from info where id = "+str(id_)
			c.execute(delsql)
			conn.commit()
			return '1'
